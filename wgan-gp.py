...
def train():
    ...
    with tf.name_scope("wgan-gp_loss"):
        gen_loss = - tf.reduce_mean(fake_logits)
        alpha = tf.random_uniform(shape=[BATCH_SIZE, 1, 1, 1], minval=0.0, maxval=1.0)
        interpolates = alpha * image + (1.0 - alpha) * fake_image               #问题出在了这
        gradients = tf.gradients(discriminator(interpolates), [interpolates,])[0]
        grad_l2 = tf.sqrt(tf.reduce_sum(tf.square(gradients), axis=[1, 2, 3]))
        gradient_penalty = tf.reduce_mean(tf.square(grad_l2 - 1.0))
        dis_loss = tf.reduce_mean(fake_logits) - tf.reduce_mean(real_logits) + LAMBDA * gradient_penalty
    with tf.name_scope("wgan-gp_optimizer"):
        ...
        d_optim = tf.train.AdamOptimizer(learning_rate=0.0001, beta1=0.0, beta2=0.9).minimize(dis_loss, var_list=dis_vars)
        g_optim = tf.train.AdamOptimizer(learning_rate=0.0001, beta1=0.0, beta2=0.9).minimize(gen_loss, var_list=gen_vars)
    ...
    if is_train:
        ...
        for i in range(num_epoches):
            for j in range(dis_iters):
                ...
                _d = sess.run(d_optim, feed_dict={z:batch_z, image:batch_image})
            batch_z = np.random.uniform(size=[BATCH_SIZE, NOISE_DIM]).astype(np.float32)
            _g= sess.run(g_optim, feed_dict={z:batch_z})
            if (i+1) % 10 == 0:
                batch_z = np.random.uniform(-1.0, 1.0, size=[BATCH_SIZE, NOISE_DIM]).astype(np.float32)
                #batch_image = sess.run(ima)
                dis_loss_data = sess.run(dis_loss, feed_dict={image:batch_image, z:batch_z})
                gen_loss_data = sess.run(gen_loss, feed_dict={image:batch_image, z:batch_z})
                print("step: {}...".format(i+1),
                      "Discriminator Loss: {:.4f}...".format(dis_loss_data),
                      "Generator Loss: {:.4f}...".format(gen_loss_data),
                      "time: {:4.4f}...".format((time.time()-start_time)))
            if (i+1) % save_rate == 0:
                if not os.path.exists('generator_images(wgan-gp)'):
                    os.mkdir(r'.\generator_images(wgan-gp)')
                ...
                samples_image = np.reshape((samples_image+1.0)*255/2.0, (ROWS, COLS, IMAGE_HEIGHT, IMAGE_WIDTH, IMAGE_DEPTH))
                #print(samples_images)
                samples_image = np.concatenate(np.concatenate(samples_image, 1), 1)
                cv2.imwrite(r'D:\gan\generator_images(wgan-gp)\%5d.jpg'%(i+1), samples_image)
        ...
        samples_image = np.concatenate(np.concatenate(samples_image, 1), 1)
        cv2.imwrite(r".\genrator_images(wgan-gp)\new_image.jpg", samples_image)
        coord.request_stop()
        coord.join(threads)
        sess.close()
...
if __name__=="__main__":
    ...
    LAMBDA = 10
    dis_iters = 5
    model_path = "./train_model(wgan-gp)"
